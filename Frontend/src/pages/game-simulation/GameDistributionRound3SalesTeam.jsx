...
const GameDistributionRound3SalesTeam = () => {
  ...
  const [retailersToVisit, setRetailersToVisit] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR3RetailersToVisit");
    return saved !== null ? parseInt(saved, 10) || 0 : 0;
  });
  const [newRetailerEffort, setNewRetailerEffort] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR3NewRetailerEffort");
  ...
  const [schemePushIntensity, setSchemePushIntensity] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR3SchemePushIntensity");
    const value = saved !== null ? parseInt(saved, 10) : 0;
    return Math.min(2, Math.max(0, Number.isNaN(value) ? 0 : value));
  }); // 0=Low, 1=Medium, 2=High
  ...
  useEffect(() => {
    localStorage.setItem("gameDistributionR3RetailersToVisit", retailersToVisit.toString());
    localStorage.setItem("gameDistributionR3NewRetailerEffort", newRetailerEffort.toString());
    localStorage.setItem("gameDistributionR3SchemePushIntensity", schemePushIntensity.toString());
  ...
  const handleBack = () => {
    navigate("/game-distribution/round3-credit-control");
  };
      ...
      <div className="w-full max-w-4xl bg-yellow-100 rounded-3xl shadow-2xl overflow-hidden border-8 border-yellow-200">
        ...
        <div className="p-8 sm:p-12">
          ...
          <div className="text-center mb-4">
            <p className="text-lg text-gray-700 leading-relaxed max-w-2xl mx-auto">
              With <span className="font-bold text-blue-700">1000 new outlets</span> to cover, your sales team deployment is critical.
              The company is reimbursing <span className="font-bold text-emerald-700">1 DSR salary (₹20,000)</span> to support expansion.
          ...
          <div className="text-center mb-4">
            <div className="inline-block bg-blue-50 border-2 border-blue-200 rounded-xl px-6 py-3">
              <p className="text-blue-800 font-bold text-lg">
                🏢 Company Support: 1 DSR salary reimbursed (₹20,000 saved on manpower cost)
              </p>
            </div>
          </div>
          ...
          <div className="text-center mb-10">
            <p className="text-md text-gray-600 italic">
              Deploy your team wisely — you need to cover both existing and new outlets effectively.
            </p>
          </div>
          ...
          <div className="flex flex-col items-center max-w-2xl mx-auto space-y-6">
            ...
            <div className="w-full bg-yellow-50 p-6 rounded-2xl border-2 border-yellow-200 shadow-sm">
              ...
              <p className="text-gray-500 text-sm italic text-center mt-3">
                Higher effort means more new outlets onboarded but reduces time with existing retailers.
              </p>
            </div>
            ...
            <div className="w-full bg-yellow-50 p-6 rounded-2xl border-2 border-yellow-200 shadow-sm">
              ...
              <p className="text-gray-500 text-sm italic text-center mt-3">
                Higher intensity improves scheme adoption in new outlets but increases sales team workload.
              </p>
            </div>
        ...
        <div className="bg-yellow-100 border-t-4 border-yellow-300 px-8 py-5 flex justify-between items-center text-lg font-bold text-gray-800">
          <div className="flex flex-col space-y-1">
            ...
            <span>Sales Team: <span className="text-emerald-700">{salesTeamAvailable}</span> + <span className="text-blue-700">1 DSR (Reimbursed)</span></span>
          </div>
...
export default GameDistributionRound3SalesTeam;
