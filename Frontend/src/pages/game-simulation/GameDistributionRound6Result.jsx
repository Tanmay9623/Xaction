...
const GameDistributionRound6Result = () => {
  ...
  // --- Round 6 Inventory (combined Opening Stock + new R6 purchases) ---
  ...
  // --- Monthly Data rows: Purchase (R6 buys) / Sale (% of OS+Purchase) / Closing ---
  const salesPercentagesR6 = { milk: 50, dark: 30, wafer: 40, gift: 40 };
  const monthlyDataRows = productRows.map(p => {
    ...
    const saleQty = Math.round((salesPercentagesR6[p.key] / 100) * combinedQty);
  ...
  // Distributor Margin: 8% - Early Payment Discount - (Total Scheme - 5%)
