...
const GameDistributionRound3Inventory = () => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const stage = searchParams.get('stage');
  
  // Uses remaining cash from Round 2
  ...
  useEffect(() => {
    ...
    localStorage.setItem("gameDistributionRound3Inventory", JSON.stringify(inventory));
    localStorage.setItem("gameDistributionStep", currentStep.toString());
  }, [cash, inventory, currentStep]);

  const handleBuy = (productId) => {
    // Navigate to the acquisition screen for the selected product, passing round=3
    navigate(`/game-distribution/acquisition?product=${productId}&round=3`);
  };

  const handleOK = () => {
    // Navigate to appropriate screen for Round 3 next step
        ...
        <div className="p-8 sm:p-12">
          ...
          <div className="bg-white border-4 border-yellow-300 rounded-xl p-6 sm:p-8 mb-10 shadow-sm max-w-4xl mx-auto space-y-6">
            <div className="text-gray-800 text-lg sm:text-xl font-medium leading-relaxed">
              <span className="font-bold text-emerald-800">As Sunshine Agency</span>, you have <span className="font-bold text-emerald-700">{formatCurrency(cash)}</span> remaining capital to invest in inventory to respond to the <span className="font-bold text-blue-600">Market Expansion Drive</span>.
            </div>
            ...
            <div className="bg-blue-50 border-l-4 border-blue-500 p-4">
              ...
              <p className="text-gray-700 leading-relaxed font-medium">
                Adjust your inventory stock keeping in mind the <span className="font-bold text-red-600">1000 new outlets</span> to be opened, and the requirement of <span className="font-bold text-emerald-700">20 units per outlet</span> for Milk Chocolate.
              </p>
            </div>
          </div>
          ...
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-gray-800">
              You have {formatCurrency(cash)} remaining to spend:
            </h2>
          </div>
...
export default GameDistributionRound3Inventory;
