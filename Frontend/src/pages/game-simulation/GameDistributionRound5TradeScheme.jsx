...
const GameDistributionRound5TradeScheme = () => {
  const navigate = useNavigate();

  // The approved scheme % from Company
  const [approvedScheme, setApprovedScheme] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR5ApprovedScheme");
  ...
  useEffect(() => {
    localStorage.setItem("gameDistributionR5QuantityDiscount", quantityDiscount.toString());
    localStorage.setItem("gameDistributionR5RetailDisplay", retailDisplayIncentive.toString());
      ...
      <div className="w-full max-w-4xl bg-yellow-100 rounded-3xl shadow-2xl overflow-hidden border-8 border-yellow-200">
        ...
        <div className="p-8 sm:p-12">
          <div className="text-center mb-6">
            <p className="text-lg text-gray-700 leading-relaxed max-w-2xl mx-auto">
              In this <span className="font-bold text-red-600">Competition Stock Out</span> phase, trade schemes can help maintain retailer loyalty even if visit frequency is slightly reduced.
              The additional approved scheme from Company is <span className="font-bold text-emerald-700">{approvedScheme}%</span> for Round 5.
            </p>
          </div>
        ...
        <div className="bg-yellow-100 border-t-4 border-yellow-300 px-8 py-5 flex justify-between items-center text-lg font-bold text-gray-800">
          ...
          <div className="flex flex-col text-right space-y-1">
            <span>Market Condition: <span className="text-red-600 font-bold uppercase">Competition Stock Out</span></span>
          </div>
        </div>
      </div>
    </div>
...
export default GameDistributionRound5TradeScheme;
