...
const GameDistributionRound4SupplyDiscipline = () => {
  ...
  const [orderFulfilmentRate, setOrderFulfilmentRate] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR4OrderFulfilment");
  ...
  const [stockBufferLevel, setStockBufferLevel] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR4StockBuffer");
    return saved !== null ? parseInt(saved, 10) || 0 : 0;
  });
  ...
  useEffect(() => {
    localStorage.setItem("gameDistributionR4OrderFulfilment", orderFulfilmentRate.toString());
    localStorage.setItem("gameDistributionR4DeliveryFrequency", deliveryFrequency.toString());
    localStorage.setItem("gameDistributionR4PriorityAllocation", priorityAllocation.toString());
    localStorage.setItem("gameDistributionR4StockBuffer", stockBufferLevel.toString());
      ...
      <div className="w-full max-w-4xl bg-yellow-100 rounded-3xl shadow-2xl overflow-hidden border-8 border-yellow-200">
        ...
        <div className="text-center pt-8 pb-4 border-b-4 border-yellow-200/50">
          <h1 className="text-4xl font-extrabold text-red-600 tracking-wider uppercase drop-shadow-sm px-4">
            Round 4 – Supply Discipline Control
          </h1>
        </div>
        ...
        <div className="p-8 sm:p-12">
          ...
          <div className="text-center mb-4">
            <p className="text-lg text-gray-700 leading-relaxed max-w-2xl mx-auto">
              With <span className="font-bold text-red-600">limited field staff</span>, aligning your delivery frequency with actual orders is crucial to avoid excess stock at retail.
            </p>
          </div>
...
export default GameDistributionRound4SupplyDiscipline;
