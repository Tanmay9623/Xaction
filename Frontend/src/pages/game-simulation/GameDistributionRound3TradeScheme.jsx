...
const GameDistributionRound3TradeScheme = () => {
  ...
  // The approved scheme % from Company (Round 3 expansion scheme)
  const [approvedScheme, setApprovedScheme] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR3ApprovedScheme");
    return saved !== null ? parseFloat(saved) : 5; // Default 5%
  });

  const [quantityDiscount, setQuantityDiscount] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR3QuantityDiscount");
    return saved !== null ? parseInt(saved, 10) || 0 : 0;
  });
  const [retailDisplayIncentive, setRetailDisplayIncentive] = useState(() => {
    const saved = localStorage.getItem("gameDistributionR3RetailDisplay");
    return saved !== null ? parseInt(saved, 10) || 0 : 0;
  });
  ...
  useEffect(() => {
    localStorage.setItem("gameDistributionR3QuantityDiscount", quantityDiscount.toString());
    localStorage.setItem("gameDistributionR3RetailDisplay", retailDisplayIncentive.toString());
      ...
      <div className="w-full max-w-4xl bg-yellow-100 rounded-3xl shadow-2xl overflow-hidden border-8 border-yellow-200">
        ...
        <div className="p-8 sm:p-12">
          ...
          <div className="text-center mb-6">
            <p className="text-lg text-gray-700 leading-relaxed max-w-2xl mx-auto">
              With <span className="font-bold text-blue-700">1000 new outlets</span> to be opened, you can offer trade schemes to incentivize new retailers and drive faster onboarding.
              ...
              The additional approved scheme from Company is <span className="font-bold text-emerald-700">{approvedScheme}%</span> for Round 3.
            </p>
          </div>

          <div className="text-center mb-10">
            <p className="text-md text-gray-600 italic">
              Adjust the scheme carefully to balance aggressive outlet expansion with profitability.
            </p>
          </div>
        ...
        <div className="bg-yellow-100 border-t-4 border-yellow-300 px-8 py-5 flex justify-between items-center text-lg font-bold text-gray-800">
          ...
          <div className="flex flex-col text-right space-y-1">
            ...
            <span>Market Condition: <span className="text-amber-600">High Demand</span></span>
          </div>
        </div>
...
export default GameDistributionRound3TradeScheme;
